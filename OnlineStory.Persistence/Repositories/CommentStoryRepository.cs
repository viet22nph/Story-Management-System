...
public class CommentStoryRepository : GenericRepository<Comment, int>, ICommentStoryRepository
{
    public CommentStoryRepository(AppDbContext context) : base(context)
    {
    }
    ...
    public async Task AddCommentAsync(Comment comment)
    {
        ...
        try
        {
            ...
            if (comment.ParentCommentId.HasValue)
            {
                ...
                UpdateBoundariesForInsert(rightValue, comment.Type, comment.StoryId);
            ...
            else
            {
                var maxRight = await _context.Comments
                    .Where(x => x.Type == comment.Type && x.StoryId == comment.StoryId)
    ...
    private void UpdateBoundariesForInsert(int rightBoundary, CommentType type, Guid? storyId)
    {
        // Increase the left and right boundaries of the existing comments
        var commentsToUpdate = _context.Comments
            .Where(c => c.Type == type && c.StoryId == storyId)
            .ToList();
    ...
    public async Task<List<CommentDto>> GetCommentByParentIdAsync(Guid storyId, int? parentId = null, int offset = 0, int limit = 50)
    {
        if (parentId is not null)
        {
            ...
            query = query.Where(x => x.Type == CommentType.Story && x.StoryId == storyId);
            query = query.Where(x => x.ParentCommentId == parentId && x.Left >= commentParent.Left && x.Right <= commentParent.Right);
    ...
    public async Task<int> CountCommentByParentIdAsync(Guid storyId, int? parentId = null)
    {
        if (storyId == Guid.Empty)
        {
            throw new ArgumentNullException("Story id empty");
        }
        IQueryable<Comment> query = _context.Comments.AsNoTrackingWithIdentityResolution();
        query = query.Where(x => x.Type == CommentType.Story && x.StoryId == storyId && x.ParentCommentId == parentId);
    ...
    public async Task RemoveCommentAsync(int commentId)
    {
        ...
        try
        {
            ...
            var listCommentsRemove = await _context.Comments
                .Where(x => x.Type == CommentType.Story && x.StoryId == comment.StoryId && x.Left >= leftValue && x.Right <= rightValue)
                .ToListAsync();
            ...
            var listComment = await _context.Comments
                .Where(x => x.Type == CommentType.Story && x.StoryId == comment.StoryId)
                .ToListAsync();
