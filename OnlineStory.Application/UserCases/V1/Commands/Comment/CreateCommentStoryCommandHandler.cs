...
public class CreateCommentStoryCommandHandler : ICommandHandler<CreateCommentStoryCommand, Success>
{
    private readonly IUnitOfWork _unitOfWork;
    public CreateCommentStoryCommandHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<Result<Success>> Handle(CreateCommentStoryCommand request, CancellationToken cancellationToken)
    {
        var comment = new Domain.Entities.Comment(request.Content, request.UserId, request.StoryId, request.ParentId);
